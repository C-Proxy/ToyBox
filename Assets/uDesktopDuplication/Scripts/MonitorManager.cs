...
namespace uDesktopDuplication
{
    ...
    {
        private static MonitorManager instance_;
        ...
        public static MonitorManager CreateInstance()
        {
            ...
            var manager = FindObjectOfType<MonitorManager>();
            if (manager)
            {
                instance_ = manager;
                return manager;
            ...
            instance_ = go.AddComponent<MonitorManager>();
            return instance_;
        }
        ...
        {
            if (id < 0 || id >= MonitorManager.monitors.Count)
            {
                Debug.LogErrorFormat("[uDD::Error] there is no monitor whose id is {0}.", id);
                return MonitorManager.primary;
            }
            return monitors[Mathf.Clamp(id, 0, MonitorManager.monitorCount - 1)];
        }
