...
using MeshForwardDirection = uDesktopDuplication.MonitorTexture.MeshForwardDirection;
...
public class MultipleMonitorCreator : MonoBehaviour
{
    ...
    public class MonitorInfo
    {
        ...
        public uDesktopDuplication.MonitorTexture uddTexture { get; set; }
    ...
    void Start()
    {
        uDesktopDuplication.MonitorManager.CreateInstance();
        Create();
    }
    ...
    void Update()
    {
        ...
        if (uDesktopDuplication.MonitorManager.monitorCount != monitors.Count)
        {
            Recreate();
        }
    }
    ...
    void OnEnable()
    {
        uDesktopDuplication.MonitorManager.onReinitialized += Recreate;
    }
    ...
    void Create()
    {
        ...
        // Create monitors
        var n = uDesktopDuplication.MonitorManager.monitors.Count;
        for (int i = 0; i < n; ++i)
        {
            // Create monitor obeject
            var go = Instantiate(monitorPrefab);
            go.name = uDesktopDuplication.MonitorManager.monitors[i].name;
            ...
            // Assign monitor
            var texture = go.GetComponent<uDesktopDuplication.MonitorTexture>();
            texture.monitorId = i;
